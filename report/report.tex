...
\subsection{Mapping Tuple Relational Calculus to First-order Predicate Logic}
  ...
  \subsection{SQL EXISTS}
    ...
    \begin{multline}
      \exists x(films\_director(x, director) \land \lnot \exists y(\lnot(films\_director(y,
      director) \land films\_length(y, length) \land length > '1:00:00')))
    \end{multline}

    \todo{Add an SQL EXCEPT section and factor this out}

    Most of this is trivial, and is covered in the previous sections. However,
    we do face an issue with the negation of the subquery. So far we have not
    handled negation. Using De Morgan's laws, we can infer the following
    equivalence.
    \begin{multline}
    ...
    Instead we must look to the query's semantic meaning. We require the return
    of the set of all tuples that do not satisfy the equation. This can be
    achieved using the SQL EXCEPT operator, which mirrors the functionality of
    the difference operator in relational algebra.
  ...
  \subsection{Testing and continuous integration.. may want to move this section}
    ...
    \subsection{Backend}
     \subsubsection{Interfacing with the databse}
      ...
      \todo{talk about other forms of error checking, not just semantic
      ...
      \missingfigure{A user makes a typo in their logic query raising a
      schema compliance semantic error. TED: The UI needs to do this!}
